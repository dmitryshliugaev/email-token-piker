...
enum ETPTokenFieldState {
    case opened
    case closed
}


@objc protocol ETPTokenFieldDelegate: UITextFieldDelegate {
    func tokenFieldShouldChangeHeight(_ height: CGFloat)
    @objc optional func tokenFieldDidSelectToken(_ token: ETPToken)
    @objc optional func tokenFieldDidBeginEditing(_ tokenField: ETPTokenField)
    @objc optional func tokenFieldDidEndEditing(_ tokenField: ETPTokenField)
}


open class ETPTokenField: UITextField {
    ...
    fileprivate var _state: ETPTokenFieldState = .opened
    ...
    fileprivate var _tokenBackgroundColor: UIColor?
    ...
    fileprivate var _direction: ETPTokenViewScrollDirection = .vertical {
        didSet {
    ...
    weak var parentView: ETPTokenView? {
        willSet (tokenView) {
            if (tokenView != nil) {
                ...
                _tokenBackgroundColor = tokenView!.tokenBackgroundColor
                _font = tokenView!.font
                if (_font != nil) {
                    font = _font
                }
                
                _placeholderFont = tokenView!.placeholderFont
    ...
    weak var tokenFieldDelegate: ETPTokenFieldDelegate? {
        didSet {
    ...
    fileprivate func _setupTokenField() {
        ...
        addTarget(self, action: #selector(ETPTokenField.tokenFieldTextDidChange(_:)), for: UIControlEvents.editingChanged)
    }
     ...
     */
    func addTokenWithTitle(_ title: String) -> ETPToken? {
    ...
    /**
     ...
     - returns: ETPToken object
     */
    func addTokenWithTitle(_ title: String, tokenObject: AnyObject?) -> ETPToken? {
    ...
    /**
     ...
     - returns: ETPToken object
     */
    func addToken(_ token: ETPToken) -> ETPToken? {
        if (token.title.characters.count == 0) {
            token.title = "Untitled"
        }
        
        if (!tokens.contains(token)) {
            token.addTarget(self, action: #selector(ETPTokenField.tokenTouchDown(_:)), for: .touchDown)
            token.addTarget(self, action: #selector(ETPTokenField.tokenTouchUpInside(_:)), for: .touchUpInside)
            tokens.append(token)
            _insertToken(token)
        }
    ...
    fileprivate func _insertToken(_ token: ETPToken, shouldLayout: Bool = true) {
    ...
    /**
     ...
     - parameter token: ETPToken object
     */
    func deleteToken(_ token: ETPToken) {
        removeToken(token)
    }
     ...
     */
    func removeToken(_ token: ETPToken, removingAll: Bool = false) {
    ...
    fileprivate func _layoutTokens() -> CGPoint {
        ...
        for token: ETPToken in tokens {
            let width = ETPUtils.getRect(token.title as NSString, width: bounds.size.width, font: _font!).size.width + ceil(_paddingX!*2+1)
    ...
    func scrollToTop() {
        let desiredOffset = CGPoint(x: 0, y: -_scrollView.contentInset.top)
        _scrollView.setContentOffset(desiredOffset, animated: true)
    }
    ...
    fileprivate func _updateText() {
        ...
        case .closed:
            if tokens.count == 0 {
                ...
                let width = ETPUtils.widthOfString(title, font: font!)
    ...
    fileprivate func _updatePlaceHolderVisibility() {
        if tokens.count == 0 && (text == ETPTextEmpty || text!.isEmpty) {
    ...
    fileprivate func _initPlaceholderLabel() {
        ...
        if (_placeholderLabel == nil) {
            _placeholderLabel = UILabel(frame: CGRect(x: xPos, y: _leftViewRect().origin.y+1, width: _selfFrame!.width - xPos - _leftViewRect().size.width, height: _leftViewRect().size.height))
    ...
    func isSelectedToken(_ token: ETPToken) -> Bool {
        if token.isEqual(selectedToken) {
            return true
        }
        return false
    ...
    func selectToken(_ token: ETPToken) {
        if (token.sticky) {
            return
        }
        for token: ETPToken in tokens {
    ...
    func tokenTouchDown(_ token: ETPToken) {
    ...
    func tokenTouchUpInside(_ token: ETPToken) {
        selectToken(token)
    }
...
extension ETPTokenField : UIScrollViewDelegate {
